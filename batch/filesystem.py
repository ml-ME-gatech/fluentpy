...
from ..fluentio import ReportFilesOut,SolutionFiles
...
class BatchFileSystem(ABC):
    ...
    def _make_solution_files(self):
        ...
        """
        if not self.solution_file_dict:
...
class TableFileSystem(BatchFileSystem):
    ...
    def __init__(self,root: str,
        ...
        self.__case_file = None
    
    @property
    def case_file(self):
        if self.__case_file is None:
            self.__case_file = self.get_case_file()
    ...
    def get_case_file(self):
        """
        assumes that there is only one .cas file and will raise 
        an error otherwise
        """
